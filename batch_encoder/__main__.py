...
def main():
    ...
    parser.add_argument(
        ...
        "-g",
        action="store_true",
        help="Generate commands and write to file",
    )
    parser.add_argument(
        "--execute", "-e", action="store_true", help="Execute commands from file"
    )
    parser.add_argument(
        "--custom",
        "-c",
        action="store_true",
        help="Customize some options for each seek",
    )
    parser.add_argument(
        "--file",
        nargs="?",
    ...
    parser.add_argument(
        "--inputfile", nargs="?", help="Set the input files separated by two commas"
    )
    parser.add_argument(
        "--loglevel",
        nargs="?",
    ...
    if mode == 1 or mode == 3:
        if args.inputfile is None:
            source_file_candidates = [
                f
                for f in os.listdir(".")
                if f.endswith(tuple(encoding_config.allowed_filetypes))
            ]
        ...
        for source_file in source_files:
            source_file_info[source_file] = SourceFile.from_file(
                source_file, encoding_config
            )
        ...
        for file, file_value in source_file_info.items():
            try:
                ...
                for seek in seek_collector.get_seek_list():
                    ...
                    logging.info(
                        f"Generating commands with seek ss: '{seek.ss}', to: '{seek.to}'"
                    )
                ...
                logging.info(
                    f"Exiting from inclusion of file '{file}' after keyboard interrupt"
                )
