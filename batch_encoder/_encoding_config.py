...
class EncodingConfig:
    ...
    def from_config(cls, config):
        allowed_filetypes = (
            config["Encoding"]
            .get(
                EncodingConfig.config_allowed_filetypes,
                EncodingConfig.default_allowed_filetypes,
            )
            .split(",")
        )
        encoding_modes = (
            config["Encoding"]
            .get(
                EncodingConfig.config_encoding_modes,
                EncodingConfig.default_encoding_modes,
            )
            .split(",")
        )
        crfs = (
            config["Encoding"]
            .get(EncodingConfig.config_crfs, EncodingConfig.default_crfs)
            .split(",")
        )
        cbr_bitrates = (
            config["Encoding"]
        ...
        cbr_max_bitrates = (
            config["Encoding"]
            .get(
                EncodingConfig.config_cbr_max_bitrates,
                EncodingConfig.default_cbr_max_bitrates,
            )
            .split(",")
        )
        threads = int(
        ...
        limit_size_enable = config.getboolean(
            "Encoding",
            EncodingConfig.config_limit_size_enable,
            fallback=EncodingConfig.default_limit_size_enable,
        )
        alternate_source_files = config.getboolean(
            "Encoding",
            EncodingConfig.config_alternate_source_files,
            fallback=EncodingConfig.default_alternate_source_files,
        )
        create_preview = config.getboolean(
            "Encoding",
            EncodingConfig.config_create_preview,
            fallback=EncodingConfig.default_create_preview,
        )
        include_unfiltered = config.getboolean(
            "Encoding",
            EncodingConfig.config_include_unfiltered,
            fallback=EncodingConfig.default_include_unfiltered,
        )
        video_filters = config.items(
            "VideoFilters", EncodingConfig.default_video_filters
        )

        default_video_stream = config["Encoding"].get(
            EncodingConfig.config_default_video_stream
        )
        default_audio_stream = config["Encoding"].get(
