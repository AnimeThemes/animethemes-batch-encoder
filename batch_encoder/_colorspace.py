...
class Colorspace(Enum):
    ...
    def value_of(source_file):
        ...
        source_colorspace = source_file.video_format["streams"][0].get(
            "color_space", ""
        )
        source_color_primaries = source_file.video_format["streams"][0].get(
            "color_primaries", ""
        )
        source_color_trc = source_file.video_format["streams"][0].get(
            "color_transfer", ""
        )
        ...
        for colorspace_candidate in Colorspace:
            if (
                ...
                logging.debug(
                    f"[Colorspace.value_of] carryover colorspace '{colorspace_candidate.name}' from source"
                )
        ...
        if resolution >= 720:
            logging.debug(
                f"[Colorspace.value_of] colorspace: '{Colorspace.HD.name}', resolution: '{resolution}'"
            )
            return Colorspace.HD
        elif resolution >= 576:
            logging.debug(
                f"[Colorspace.value_of] colorspace: '{Colorspace.PAL.name}', resolution: '{resolution}'"
            )
            return Colorspace.PAL
        else:
            logging.debug(
                f"[Colorspace.value_of] colorspace: '{Colorspace.NTSC.name}', resolution: '{resolution}'"
            )
